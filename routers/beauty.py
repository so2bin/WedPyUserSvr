...
class HTTPHandler(BaseHandler):
    ...
    def doPost(cls, request):
        ...
        if vBrght is None and vCntrst is None:
            return cls.jsonResponse(request, {'msg': 'params error: brightness and contrast both is None', 'status': 1}, 400)
        if not data:
            return cls.jsonResponse(request, {'msg':'empty data', 'status': 1}, 400)
        if not data.get('originAddr'):
            return cls.jsonResponse(request, {'msg': 'no originAddr', 'status': 1}, 400)
        if not data.get('targetAddr'):
            return cls.jsonResponse(request, {'msg': 'no targetAddr', 'status': 1}, 400)
        ...
        if data.get('saveStartNo'):
            if imgNo.RE_OBJ_IMG_NO.match(data.get('saveStartNo')):
                ...
                return cls.jsonResponse(request, {'msg': 'Invalid StartNo: %s' % data['saveStartNo'], 'status': 1}, 400)

        if not os.path.exists(addr):
            return cls.jsonResponse(request, {'msg': 'not a valid image file or folder', 'status': 1}, 400)
        if not os.path.isdir(toAddr):
            return cls.jsonResponse(request, {'msg': 'targetAddr must be folder', 'status': 1}, 400)
        ...
        if os.path.isfile(addr):
            ...
            if imgExt not in IMG_SUPPORTS:
                return cls.jsonResponse(request, {'msg': 'only support image formats: %s' % IMG_SUPPORTS, 'status': 1}, 400)
            if not svImgExt:
                ...
                if os.path.isfile(fpath):
                    ...
                    if imgExt in IMG_SUPPORTS:
                        ...
                        if not svImgExt:
                            svImgExt = DEFAULT_IMG_EXT
        if not imgUrls:
            return cls.jsonResponse(request, {'msg': 'not find images with the path', 'status': 1}, 400)
        if not svImgExt:
            svImgExt = DEFAULT_IMG_EXT
